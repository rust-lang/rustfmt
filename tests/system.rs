...
fn assert_output(source: &str, expected_filename: &str, write_mode: WriteMode) {
    let (config, run_config) = read_config(&source);
    let file_map = run_rustfmt(source.to_string(), write_mode);

    // Populate output by writing to a vec.
    let mut out = vec![];
    let _ = filemap::write_all_files(&file_map, &mut out, write_mode, &config, &run_config);
...
fn read_config(filename: &str) -> (Config, RunConfig) {
    ...
    for (key, val) in &sig_comments {
        if key != "target" && key != "config" {
            if key.starts_with("run_config-") {
                run_config.override_value(&key["run_config-".len()..], val);
            } else {
...
fn run_rustfmt(filename: String, write_mode: WriteMode) -> FileMap {
    let (config, run_config) = read_config(&filename);
    format(Path::new(&filename), &config, &run_config, write_mode)
}
...
pub fn idempotent_check(filename: String,
    ...
    let format_report = fmt_lines(&mut file_map, &config, &run_config);

    let mut write_result = HashMap::new();
    for (filename, text) in file_map.iter() {
        let mut v = Vec::new();
        // Won't panic, as we're not doing any IO.
        write_system_newlines(&mut v, text, &config, &run_config).unwrap();
