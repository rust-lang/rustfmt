...
enum Operation {
    ...
    ConfigOutputEditorconfig { editorconfig_path: Option<PathBuf> },
...
fn make_opts() -> Options {
    ...
    let help_topics = if is_nightly {
        "`config`, `file-lines` or `editorconfig`"
    } else {
        "`config`"
    };
...
fn execute(opts: &Options) -> Result<i32> {
    ...
    match determine_operation(&matches)? {
        ...
        Operation::Help(HelpOp::EditorConfig) => {
            print_help_editorconfig();
            Ok(0)
        }
        Operation::Version => {
            print_version();
        ...
        Operation::ConfigOutputEditorconfig { editorconfig_path } => output_editorconfig(
...
fn print_help_editorconfig() {
    let version = rustfmt::version_str();
    println!(
        "To append the generated EditorConfig to a specific file or directory
...
```
    ...
    cat ./my_dir/.editorconfig

    # Generated by rustfmt version: {version}
...
fn determine_operation(matches: &Matches) -> Result<Operation, OperationError> {
    if matches.opt_present("h") {
        ...
        return match topic.as_str() {
            ...
            "editorconfig" if is_nightly() => Ok(Operation::Help(HelpOp::EditorConfig)),
    ...
    if matches.opt_present("editorconfig") {
        return Ok(Operation::ConfigOutputEditorconfig {
            editorconfig_path: matches.opt_get("editorconfig").unwrap(),
        });
    }
